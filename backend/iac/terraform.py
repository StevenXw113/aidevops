...
def _normalize_object_storage(cloud_provider, config):
    ...
    for index, bucket in enumerate(bucket_sources):
        ...
        normalized_buckets.append({
            'bucket_name': _normalize_bucket_name(_first_non_empty(candidate.get('bucket_name'), bucket_defaults.get('bucket_name'), f'aidevops-bucket-{index + 1}')),
...
def _build_huaweicloud_main_tf(payload):
    ...
    lines = [
        ...
        'resource "huaweicloud_networking_secgroup" "this" {', f'  name        = {_hcl_string(payload["name"] + "-sg")}', '  description = "Managed by AiDevOps Terraform generator"', '}', '',
...
def _build_readme(payload, has_secret_tfvars):
    ...
    return '\n'.join([f'# {payload["display_name"] or payload["name"]}', '', payload['provider_meta']['description'], '', '## 资源清单', '', *summary_lines, '', '## 使用方式', '', *usage, '', '## 说明', '', '- 该工程由 AiDevOps 自动生成。', '- 敏感凭证不会持久化到 AiDevOps 数据库。', '- 如需更复杂的路由、监听器、白名单或参数组，请在生成文件基础上继续扩展。', ''])
