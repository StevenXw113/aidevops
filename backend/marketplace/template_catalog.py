...
services:
  {service_name}:
    image: {image}
    container_name: aidevops_{service_name}
    restart: always
...
services:
  postgres:
    ...
    container_name: aidevops_postgres
    restart: always
    ports:
      - "{{port}}:5432"
    environment:
...
services:
  mongodb:
    image: mongo:{{version}}
    container_name: aidevops_mongodb
    restart: always
    ports:
      - "{{port}}:27017"
    environment:
...
services:
  jenkins:
    ...
    container_name: aidevops_jenkins
    restart: always
    ports:
      - "{{port}}:8080"
      - "50000:50000"
...
services:
  gitlab:
    ...
    container_name: aidevops_gitlab
    restart: always
    ports:
      - "{{http_port}}:80"
      - "{{ssh_port}}:22"
...
services:
  grafana:
    ...
    container_name: aidevops_grafana
    restart: always
    ports:
      - "{{port}}:3000"
    volumes:
...
services:
  elasticsearch:
    ...
    container_name: aidevops_elasticsearch
    restart: always
    ports:
      - "{{port}}:9200"
    environment:
...
''',
    },
    {
        ...
        'env_schema': [
            ...
            {'key': 'secret_key', 'label': 'Secret Key', 'default': 'aidevops_jumpserver_secret', 'required': True},
        ],
        'docker_compose_template': '''version: "3.8"
services:
  jumpserver:
    image: jumpserver/jms_all:{{version}}
    container_name: aidevops_jumpserver
    restart: always
    ports:
      - "{{port}}:80"
      - "2222:2222"
...
services:
  xxl-job-admin:
    ...
    container_name: aidevops_xxljob
    restart: always
    ports:
      - "{{port}}:8080"
    environment:
